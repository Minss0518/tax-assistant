...
- `reset_law_api_collection()`은 파이프라인 실행당 **한 번만** `tax_law_api_v1` 컬렉션을
  ...
  대신 전체 재구축 방식). 실제 문서 추가는 `index_chunks_batch()`로 작은 배치(기본 50건)
  단위로 나눠서 처리합니다 — 판례만 1000건이 넘는데 전체를 한 번에 메모리에 올려서
  임베딩하면 Render 무료 인스턴스(512MB)에서 **실제로 OOM으로 프로세스가 강제 종료되는 것을
  확인**했습니다. 배치 크기는 `run_pipeline.py`의 `INDEX_BATCH_SIZE`로 조절할 수 있습니다.
