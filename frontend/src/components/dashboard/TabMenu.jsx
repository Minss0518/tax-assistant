    ...
    <>
      <style>{`
        @media (max-width: 480px) {
          ...
          .tab-menu-wrap { padding: 8px !important; }
        }
      `}</style>
      <div className="tab-menu-wrap" style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: "10px 12px", marginBottom: 14 }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: "0 16px" }} onClick={() => setShowModal(false)}>
          <div style={{ background: "white", borderRadius: 24, padding: 32, maxWidth: 360, width: "100%", textAlign: "center", boxShadow: "0 25px 50px rgba(0,0,0,0.2)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowModal(false)} style={{ flex: 1, padding: "12px 0", borderRadius: 12, border: "1px solid #e5e7eb", background: "white", color: "#6b7280", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>취소</button>
              <button onClick={() => { setShowModal(false); navigate("/pricing"); }} style={{ flex: 1, padding: "12px 0", borderRadius: 12, border: "none", background: "#7c3aed", color: "white", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>확인</button>
            </div>
          </div>
        </div>
      )}
