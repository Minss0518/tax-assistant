...
export default function ChatPage() {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                setMessages((prev) => {
                    ...
                    updated[updated.length - 1] = {
                        role: 'assistant',
                        content: `⚠️ ${errData.detail || '오류가 발생했어요.'}`
                    };
                    return updated;
                });
                setLoading(false);
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (json.token) {
                            setMessages((prev) => {
                                const updated = [...prev];
                                updated[updated.length - 1] = { role: 'assistant', content: updated[updated.length - 1].content + json.token };
                                return updated;
                            });
                        }
                    } catch (e) { }
            ...
            setMessages((prev) => {
                const updated = [...prev];
                updated[updated.length - 1] = { role: 'assistant', content: '오류가 발생했어요. 다시 시도해 주세요.' };
                return updated;
            });
        } finally {
            setLoading(false);
                    ...
                    {messages.map((msg, i) => {
                        ...
                        return (
                            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                ...
                                <div className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-white text-gray-700 shadow-sm border border-gray-100 rounded-bl-sm'}`}>
                                    {msg.role === 'assistant' ? (
