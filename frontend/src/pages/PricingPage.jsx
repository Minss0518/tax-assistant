      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-gray-500 text-sm mb-6 leading-relaxed">Pro 플랜 결제 기능을 열심히 준비하고 있어요.<br />출시되면 가장 먼저 알려드릴게요!</p>
            <button onClick={() => setShowModal(false)} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold text-sm transition">확인</button>
          </div>
        </div>
      )}
    </div>
