                ...
                <div className="flex flex-col gap-4 mb-8">
                    {plans.map((plan, i) => (
                        <div key={i} className={`bg-white rounded-2xl border-2 ${plan.color} p-6 relative`}>
                            {plan.badge && (
                                <span className="absolute -top-3 left-6 bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full">{plan.badge}</span>
                            )}
                            ...
                            <button disabled={plan.current} onClick={() => !plan.current && setShowModal(true)}
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" onClick={() => setShowModal(false)}>
                    <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl" onClick={(e) => e.stopPropagation()}>
                        ...
                        <h3 className="font-bold text-gray-900 text-xl mb-2">Pro 플랜 시작하기</h3>
                        <p className="text-gray-500 text-sm mb-6 leading-relaxed">월 9,900원으로 AI 세무 상담, OCR, 리포트를<br />무제한으로 사용할 수 있어요.</p>
                        <button
                            onClick={handlePayment}
                            className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold text-sm transition mb-2"
                        >
                            💳 카드로 결제하기 (월 9,900원)
                        </button>
                        <button
