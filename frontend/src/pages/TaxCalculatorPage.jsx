...
const LONGCARE_RATE = 0.1295;
const PENSION_MAX_INCOME = 61700000;

function calcIncomeTax(taxableIncome) {
  if (taxableIncome <= 0) return 0;
  for (const b of TAX_BRACKETS) {
    if (taxableIncome <= b.limit) return Math.max(0, Math.floor(taxableIncome * b.rate - b.deduction));
  }
  return 0;
}
...
export default function TaxCalculatorPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setIncome(''); setExpense(''); setBasicDeduction('1500000');
      setAutoInsurance(true); setPensionDeduction(''); setHealthDeduction('');
      setOtherDeduction(''); setResult(null);
    }
  }, [userId]);

  useEffect(() => {
    if (!userId) return;
    localStorage.setItem(`taxInput_${userId}`, JSON.stringify({ income, expense, basicDeduction, autoInsurance, pensionDeduction, healthDeduction, otherDeduction }));
  ...
  const handleCalculate = async () => {
    ...
    if (autoInsurance) {
      pensionNum = Math.floor(Math.min(netIncome, PENSION_MAX_INCOME) * PENSION_RATE);
    ...
    try {
      await api.post('/tax-calculator/save', { grossIncome: incomeNum, expense: expenseNum, taxableIncome, incomeTax: finalIncomeTax, localTax, totalTax, prepaidTax: withheld, finalTax, isRefund, refundAmount: isRefund ? finalTax : 0 });
  ...
  const handleReset = async () => {
    ...
    try { await api.delete('/tax-calculator/clear'); } catch (e) { console.error(e); }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <style>{`
        @media (max-width: 480px) {
          .tax-result-row { font-size: 12px !important; }
          .tax-final-amount { font-size: 22px !important; }
        }
      `}</style>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 12px 48px' }}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-4">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div><label className={LABEL_CLASS}>필요경비</label>
              <input className={INPUT_CLASS} type="number" placeholder="예: 5000000" value={expense} onChange={(e) => setExpense(e.target.value)} /></div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-4">
          ...
          <div className="flex flex-col gap-3">
            <div><label className={LABEL_CLASS}>기본공제</label>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
                ...
                <div className="flex gap-1">
                  <button onClick={() => setAutoInsurance(true)} className={`text-xs px-3 py-1 rounded-lg font-semibold transition ${autoInsurance ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'}`}>자동계산</button>
                  <button onClick={() => setAutoInsurance(false)} className={`text-xs px-3 py-1 rounded-lg font-semibold transition ${!autoInsurance ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'}`}>직접입력</button>
                </div>
              </div>
              {autoInsurance ? (
                ...
                <div className="flex flex-col gap-2">
                  <input className={INPUT_CLASS} type="number" placeholder="국민연금 납부액 (원)" value={pensionDeduction} onChange={(e) => setPensionDeduction(e.target.value)} />
                  <input className={INPUT_CLASS} type="number" placeholder="건강보험료 납부액 (원)" value={healthDeduction} onChange={(e) => setHealthDeduction(e.target.value)} />
                </div>
              )}
            </div>
            <div><label className={LABEL_CLASS}>기타공제</label>
              <input className={INPUT_CLASS} type="number" placeholder="예: 500000" value={otherDeduction} onChange={(e) => setOtherDeduction(e.target.value)} /></div>
          </div>
        </div>

        <div className="bg-violet-50 rounded-2xl px-4 py-3 mb-4 text-xs text-violet-500">
          <p className="font-semibold mb-1">자동 적용 세액공제</p>
          <p>• 표준세액공제 70,000원 자동 반영</p>
        </div>

        <div className="flex gap-2 mb-6">
          <button onClick={handleCalculate} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-semibold text-sm transition">계산하기</button>
          <button onClick={handleReset} className="px-5 bg-gray-100 hover:bg-gray-200 text-gray-500 py-3 rounded-xl font-semibold text-sm transition">초기화</button>
        </div>
        ...
        {result && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="flex flex-col gap-0 mb-4">
              {[
                { label: '총 수입금액', value: fmt(result.incomeNum) },
                ...
                <div key={i} className={`tax-result-row flex justify-between text-sm py-2 border-b border-gray-50 last:border-0 ${row.bold ? 'font-semibold' : ''}`}>
            ...
            <div className="bg-gray-50 rounded-xl p-4 flex flex-col gap-2 mb-4">
              {[
                ...
                <div key={i} className={`tax-result-row flex justify-between text-sm py-1 ${row.bold ? 'font-semibold' : ''}`}>
                  <span className="text-gray-500">{row.label}</span>
                  <span className={row.color || 'text-gray-700'}>{row.value}원</span>
                </div>
              ))}
            ...
            <div className={`rounded-xl p-4 text-center ${result.isRefund ? 'bg-green-50 border border-green-100' : 'bg-red-50 border border-red-100'}`}>
              ...
              <p className={`tax-final-amount text-2xl font-bold ${result.isRefund ? 'text-green-500' : 'text-red-500'}`}>{fmt(result.finalTax)}원</p>
